...
def leaderboard(request):
    ...
    for user in users:
        ...
        user_data.append({
            ...
            user.username,
            'level':
            user_leaderboard_data.level if user_leaderboard_data else 1,
            'quiz_count':
            user_leaderboard_data.quiz_count if user_leaderboard_data else 0
        })
    ...
    current_user_rank = user_data.index(
        {
            ...
            'quiz_count':
            current_user_data.quiz_count if current_user_data else 0
        }) + 1
